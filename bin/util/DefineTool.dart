...
class DefineTool {
    ...
    for (int i = 0; i < listFile.length; i++) {
      ...
      if (stat.type == FileSystemEntityType.directory) {
        ...
        if (fileNameList.contains(endName)) {
          List<int> data = FileUtil.readData(file.path);
          String code = utf8.decode(data);
          if (code.contains("#ifdef") ||
              code.contains("#else") ||
              code.contains("#endif") ||
              code.contains("#ifndef")) {
  ...
  KeyWordItem? getNextLineWordItem(CodeParseUtil parseUtil, KeyWordItem item) {
    ...
    for (; index < parseUtil.list_keyWordItems.length; index++) {
      ...
      if (nextItem != null &&
          nextItem.type == KeyWordType.TYPE_BLOCK_ANNOTATION &&
          keyWordItem.type == KeyWordType.TYPE_LINE) {
        return nextItem;
      }
      if (isNextLine) {
        return keyWordItem;
  ...
  //寻找上一行的KeyWordItem
  KeyWordItem? getForwardLineWordItem(
      CodeParseUtil parseUtil, KeyWordItem item) {
    int index = parseUtil.list_keyWordItems.indexOf(item);
    index -= 1;
    bool isNextLine = false;
    ...
    for (int i = 0; i < parseUtil.list_keyWordItems.length; i++) {
      ...
      if (keyWordItem.type == KeyWordType.TYPE_ANNOTATION) {
        ...
        if (keyWordItem.word.contains("#ifdef")) {
          ...
          if (definesList.length >= 1) {
            ...
            if (isContains) {
              ...
              if (wordItem!.word.startsWith("/*")) {
                wordItem.word =
                    wordItem.word.substring(2, wordItem.word.length);
          ...
          // print(" isAddAnnotation = "+isAddAnnotation.toString() + " isRemoveAnnotation = "+isRemoveAnnotation.toString() + " defineState = "+defineState.toString() + " isContains = "+isContains.toString());
          if (isAddAnnotation) {
            ...
            if (wordItem!.word.endsWith("*/")) {
              wordItem.word =
                  wordItem.word.substring(0, wordItem.word.length - 2);
            }
